...
type AppScreen = 'login' | 'dashboard' | 'customerDashboard' | 'menuManagement' | 'filterMenu';
...
export default function App() {
  ...
  const calculateAveragePrices = () => {
    const averages: Record<CourseType, number> = {
      starters: 0,
      mains: 0,
      desserts: 0,
      drinks: 0,
    ...
    courses.forEach(course => {
      const items = getItemsByCourse(course.value);
      if (items.length > 0) {
        const total = items.reduce((sum, item) => sum + item.price, 0);
        averages[course.value] = total / items.length;
      }
    });
            ...
            <Card style={styles.headerCard}>
              <Card.Content>
                ...
                <View style={styles.buttonRow}>
                  <Button
                    mode="outlined"
                    onPress={() => setCurrentScreen('filterMenu')}
      ...
      <View style={[styles.container, { backgroundColor: customTheme.colors.background }]}>
        <Animated.View style={[styles.animatedContainer, { opacity: fadeAnim }]}>
          <ScrollView>
            <Card style={styles.headerCard}>
              <Card.Content>
                ...
                <View style={styles.buttonRow}>
                  <Button
                    mode="outlined"
                    onPress={() => setCurrentScreen('menuManagement')}
                    style={styles.navButton}
                    icon="plus"
                  >
                    Manage Menu
            ...
            {/* Display full menu for overview */}
            {courses.map(course => {
              const courseItems = getItemsByCourse(course.value);
              if (courseItems.length === 0) return null;
  ...
  // Render menu management screen for chefs
  ...
  return (
    <PaperProvider theme={customTheme}>
      ...
      {currentScreen === 'menuManagement' && role === 'chef' && renderMenuManagement()}
      {currentScreen === 'filterMenu' && role === 'customer' && renderFilterMenu()}
    </PaperProvider>
  );
}
...
const styles = StyleSheet.create({
  ...
  // New styles for averages and navigation
  averagesContainer: {
    marginTop: 12,
    marginBottom: 12,
  },
